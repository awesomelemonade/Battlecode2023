...
pub fn random_micro(state: &mut GameState, id: u32) {
    let mut rng = rand::thread_rng();
    let dir = *ALL_DIRS.choose(&mut rng).unwrap();
...
pub fn get_scored_micro(coeffs: [f32; 12]) -> impl Fn(&mut GameState, u32) -> () {
    move |state, id| {
...
pub fn sprint_micro(state: &mut GameState, id: u32) {
    ...
    fn get_score_for_kiting(state: &GameState, id: u32, pos: Position) -> f32 {
        let robot = state.robots.get(&id).unwrap();
        let sensed_robots = state.sense_nearby_robots(id);
