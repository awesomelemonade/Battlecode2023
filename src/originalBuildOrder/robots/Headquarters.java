package originalBuildOrder.robots;

import battlecode.common.*;
import originalBuildOrder.fast.FastDoubleTracker;
import originalBuildOrder.fast.FastIntMap;
import originalBuildOrder.fast.FastIntTracker;
import originalBuildOrder.util.*;

import java.util.function.ToIntFunction;

import static originalBuildOrder.util.Constants.rc;
    ...
    public static Communication.CarrierTaskType getNewTask() {
        ...
        if (nearestEnemyHQ == null) {
            ...
            if (distance > 25 || Cache.ALLY_ROBOTS.length >= 20) {
                Debug.setIndicatorString("Standard 1");
                if (Math.random() < 0.5) {
                    return Communication.CarrierTaskType.MINE_ADAMANTIUM;
                } else {
                    return Communication.CarrierTaskType.MINE_MANA;
                ...
                if (rc.getRoundNum() <= numManaRounds) {
                    Debug.setIndicatorString("Rushing");
                    return Communication.CarrierTaskType.MINE_MANA;
                } else {
                    Debug.setIndicatorString("Standard 2");
                    if (Math.random() < 0.5) {
                        return Communication.CarrierTaskType.MINE_ADAMANTIUM;
                    } else {
                        return Communication.CarrierTaskType.MINE_MANA;
